...
if [ $stage -le 6 ] && [ $stop_stage -ge 6 ]; then
    if $extract_video_feature; then 
        ...
        if [ $segment_type = wrd ]; then
            ...
            cp -r $tgt_dir/feat/onehot_clus${n_sp_clus}_given_phn $tgt_dir/feat/onehot_clus${n_sp_clus}_given_phn_vid_cpc_clus${n_vid_clus}
            
            for x in train valid test; do
#                cp $tgt_cpc_dir/CLUS$n_vid_clus/$x.src $tgt_cpc_dir/CLUS$n_vid_clus/$x.wrd
#                cp $tgt_dir/feat/onehot_clus${n_sp_clus}_given_phn_vid_cpc_clus${n_vid_clus}/$x.tsv $tgt_cpc_dir/CLUS$n_vid_clus
                cp $tgt_cpc_dir/CLUS$n_vid_clus/$x.wrd $tgt_dir/feat/onehot_clus${n_sp_clus}_given_phn_vid_cpc_clus${n_vid_clus}
            done
        else

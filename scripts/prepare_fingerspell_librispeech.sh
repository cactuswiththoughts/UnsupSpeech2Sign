...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    # get manifest
    python ${FAIRSEQ_ROOT}/examples/wav2vec/wav2vec_manifest.py ${LIBRI_ROOT}/dev-clean --ext flac --dest ${tgt_dir} --valid-percent 0
    mv $(pwd)/${tgt_dir}/train.tsv $(pwd)/${tgt_dir}/dev.tsv
    python ${FAIRSEQ_ROOT}/examples/wav2vec/wav2vec_manifest.py ${LIBRI_ROOT}/test-clean --ext flac --dest ${tgt_dir} --valid-percent 0
    mv $(pwd)/${tgt_dir}/train.tsv $(pwd)/${tgt_dir}/test.tsv
    python ${FAIRSEQ_ROOT}/examples/wav2vec/wav2vec_manifest.py ${LIBRI_ROOT}/train-clean-100 --ext flac --dest ${tgt_dir} --valid-percent 0
    ...
    python ${FAIRSEQ_ROOT}/fairseq/examples/wav2vec/wav2vec_manifest.py ${tgt_dir}/no_silence --ext flac --dest ${tgt_dir}_ns --valid-percent 0.01
...
if [ $stage -le 3 ] && [ $stop_stage -ge 3 ]; then 
    ...
    ch2img_path=$(pwd)/manifest/asl_alphabet/ch2img.json

...
class ASL_LibriSpeech_Visualizer:
    def __init__(
        ...
        with open(retrieval_file, "r") as f:
            ...
            self.alignments = ret_dict["alignments"]
            self.dist_mats = ret_dict["dist_mats"]

        self.audio_data = []
        with open(speech_manifest_dir / f"{split}.tsv", "r") as f_tsv,\
            open(speech_manifest_dir / f"{split}.jsonlines", "r") as f_json,\
            open(speech_manifest_dir / f"{split}.wrd", "r") as f_wrd:
            lines_tsv = f_tsv.read().strip().split("\n")
            _ = lines_tsv.pop(0)
            lines_json = f_json.read().strip().split("\n")
            lines_wrd = f_wrd.read().strip().split("\n")
            for l_tsv, l_json, l_wrd in zip(lines_tsv, lines_json, lines_wrd):
                audio_path = l_tsv.split("\t")[0]
                audio_dict = json.loads(l_json)
                tran = l_wrd.strip().split()
                self.audio_data.append(
                    (audio_path, audio_dict, tran)
                )
    ...
    def show_similarity_map(
        ...
        if find_correct:
            idx = self.correct_indices[idx]
        audio_path = Path(self.audio_data[idx][0])
        fname = audio_path.name.split(".")[0]
        align_indices = self.alignments[idx][0]
        tran = self.audio_data[idx][2]
        dist_mat = self.dist_mats[idx][0]
        S = -np.asarray(dist_mat) 
        
        print(f"Sample idx: {idx}")
        plot_similarity(S, tran)
        plt.savefig(self.out_root / f"{fname}_similarity_map.png")

    def show_speech(self, idx, find_correct=True):
        if find_correct:
            idx = self.correct_indices[idx]
        y, sr = read_speech(*self.audio_data[idx][:2])
    ...
    def show_video(self, idx, find_correct=True):
        if find_correct:
            idx = self.correct_indices[idx]
        audio_path = Path(self.audio_data[idx][0])
        fname = audio_path.name.split(".")[0]
    ...
    def show_optical_flow(self, idx, find_correct=True):
        if find_correct:
            idx = self.correct_indices[idx]
        audio_path = Path(self.audio_data[idx][0])
        fname = audio_path.name.split(".")[0]
...
def plot_similarity(S, tran):
    ...
    ax.set_xticklabels(tran, rotation=15)
...
if __name__ == "__main__":
    retrieval_file = "~/UnsupSpeech2Sign/multirun/l1_w2vu_word_500_segmented_onehot_clus2000_asl_i3d_joint_charades_cpc_clus500/0/retrieval.json" 
    video_root = "~/MS-ASL/downloads"
    flow_root = "~/UnsupSpeech2Sign/manifest/MS-ASL/i3d_joint_charades_mean"
    speech_manifest_dir="~/UnsupSpeech2Sign/manifest/asl_librispeech960_500words" 
    sent_manifest_dir="~/UnsupSpeech2Sign/manifest/asl_librispeech960_500words/asl_feat/i3d_joint_charades_cpc_npredicts3_32negatives"
    word_manifest_dir="~/UnsupSpeech2Sign/manifest/MS-ASL/i3d_joint_charades_mean_cpc"
    out_root="~/UnsupSpeech2Sign/visualizations_500"
    ...
    for idx in range(min(5, len(visualizer.correct_indices))): 
        print(f"Show similarity map for sample {idx}", flush=True)
        visualizer.show_similarity_map(idx)
        print("Show mel spectrogram", flush=True)
#        visualizer.show_speech(idx)
        print("Show video", flush=True)
#        visualizer.show_video(idx)
        print("Show optical flow", flush=True)

...
def get_parser():
    ...
    parser.add_argument('--reduction', default='mean', help='reduction method from frame-level features to segment-level features')
...
class Wav2VecFeatureReader(object):
    ...
    def get_feats(self, loc, ali, reduction):
    ...
    with open(osp.join(args.data, args.split) + ".tsv", "r") as fp,\
        ...
        def iterate():
            for fname, ali in zip(files, alis):
                w2v_feats = reader.get_feats(fname, ali, reduction=args.reduction)
                yield w2v_feats

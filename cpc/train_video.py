...
def trainStep(dataLoader,
    ...
    for step, fulldata in enumerate(dataLoader):
        ...
        if (step + 1) % loggingStep == 0:
            ...
            print(f"Update {step + 1}", flush=True)
            print(f"elapsed: {elapsed:.1f} s", flush=True)
            print(
                f"{1000.0 * elapsed / loggingStep:.1f} ms per batch, {1000.0 * elapsed / n_examples:.1f} ms / example", flush=True)
...
def run(trainDataset,
    ...
    trainLoader = trainDataset.getDataLoader(
        batchSize, numWorkers=4, shuffle=True,
    )

    valLoader = valDataset.getDataLoader(
        batchSize, numWorkers=4, shuffle=False,
    )
    ...
    for epoch in range(startEpoch, nEpoch):
        ...
        print("Training dataset %d batches, Validation dataset %d batches, batch size %d" %
              (len(trainLoader), len(valLoader), batchSize), flush=True)

        locLogsTrain = trainStep(trainLoader, cpcModel, cpcCriterion,
                                 optimizer, scheduler, logs["logging_step"], 
                                 debug=debug)

        locLogsVal = valStep(valLoader, cpcModel, cpcCriterion, debug=debug)
...
def main(args):
    ...
    if args.pathCheckpoint is not None and not args.restart:
        ...
        if cdata is not None:
            ...
            print(f"Checkpoint detected at {data}", flush=True)
    ...
    with open(os.path.join(args.pathCheckpoint, 'checkpoint_args.json'), 'w') as file:
        json.dump(args.__dict__, file)

    logs["logging_step"] = args.logging_step

    print(f'CONFIG:\n{json.dumps(vars(args), indent=4, sort_keys=True)}', flush=True)
    print('-' * 50, flush=True)

    data_path = Path(args.pathDB)
    feat_path = Path(args.pathFeat)
    trainDataset = VideoBatchData(data_path / "train.trn",
                                  args.wrd2vid_path,
                                  feat_path,
                                  max_keep_sample_size=args.max_keep_sample_size,
                                  min_keep_sample_size=args.min_keep_sample_size,
                                  max_sample_size=args.max_sample_size)
    print("Training dataset loaded")
    print("")

    print("Loading the validation dataset")
    valDataset = VideoBatchData(data_path / "dev.trn",
                                args.wrd2vid_path,
                                feat_path,
                                max_keep_sample_size=args.max_keep_sample_size,
                                min_keep_sample_size=args.min_keep_sample_size,
    ...
    if args.load is not None and args.loadCriterion:
        cpcCriterion = loadCriterion(
            args.load[0], cpcModel.gEncoder.DOWNSAMPLING, 0, 26,
        )
    else:
        cpcCriterion = getCriterion(
            args, cpcModel.gEncoder.DOWNSAMPLING, 0, 26,
        )
...
def parseArgs(argv):
    ...
    group_db.add_argument('--pathDB', type=str, default=None,
                          ...
                          'data.')
    group_db.add_argument('--pathFeat', type=str, default=None,
                          help='Path to the directory containing the '
                          'features.')
    group_db.add_argument('--wrd2vid_path', type=str, default=None,
                          help='Path to the file containing the mapping '
                          'from words to videos.')
    group_db.add_argument('--length_path', type=str, default=None,
                          help='Path to the file containing the  '
                          'video feature lengths.')
    group_db.add_argument('--max_keep_sample_size', type=int, default=1000,
                          help='Maximal sentence length'
                          'in characters.')
    group_db.add_argument('--min_keep_sample_size', type=int, default=10,
                          help='Minimal sentence length'
                          'in characters.')
    group_db.add_argument('--max_sample_size', type=int, default=256,
    ...
    group_gpu.add_argument('--nGPU', type=int, default=-1,
                           ...
                           "available GPUs)")
    group_gpu.add_argument('--batchSizeGPU', type=int, default=64,
                           help='Number of batches per GPU.')
 
    parser.add_argument('--inputEncoder', type=int, default=512,
                        help="Input dimension of the CPC Encoder")
    ...
    if args.pathCheckpoint is not None:
        if not os.path.exists(args.pathCheckpoint):
            os.makedirs(args.pathCheckpoint)

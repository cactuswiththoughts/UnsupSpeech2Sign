...
class FeatureModule(torch.nn.Module):
    ...
    def __init__(self, featureMaker, get_encoded, collapse=False, device="cuda"):
    ...
    def forward(self, data):
        ...
        cFeature, encoded, _ = self.featureMaker(
            batchAudio.to(self.device), label,
        )
        if self.get_encoded:
            cFeature = encoded
        if self.collapse:
...
def getEncoder(args):
    ...
    elif args.encoder_type == 'ffn':
        from .model import FFNEncoder 
        return FFNEncoder(
            args.inputEncoder, args.hiddenEncoder, args.dropout,
        )
    else:

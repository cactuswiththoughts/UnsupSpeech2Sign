...
def run(trainDataset,
    ...
    trainLoader = trainDataset.getDataLoader(
        batchSize, numWorkers=0, shuffle=True,
    )

    valLoader = valDataset.getDataLoader(
        batchSize, numWorkers=0, shuffle=False,
    )
...
def main(args):
    ...
    trainDataset = ImageBatchData(data_path / "train.trn",
                                  args.ch2img_path,
                                  feat_path,
    ...
    valDataset = ImageBatchData(data_path / "dev.trn",
                                args.ch2img_path,
                                feat_path,
                                max_keep_sample_size=args.max_keep_sample_size,
                                min_keep_sample_size=args.min_keep_sample_size,
...
def parseArgs(argv):
    ...
    group_db.add_argument('--pathFeat', type=str, default=None,
                          help='Path to the directory containing the '
                          'features.')
    group_db.add_argument('--ch2img_path', type=str, default=None,
    ...
    group_gpu.add_argument('--nGPU', type=int, default=-1,
                           ...
                           "available GPUs)")
    group_gpu.add_argument('--batchSizeGPU', type=int, default=4,
